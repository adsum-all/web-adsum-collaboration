...
Collaboration workspace (Trello-like boards, instruction channel, calendar, published
org chart in read-only): spaces, boards, cards, checklists, attachments, notifications.

## Stack

React 18 + TypeScript, built with Vite. The published organisation chart reuses the
back-office engine @xyflow/react (React Flow) with a dagre layout, in read-only mode.
No Tailwind or component kit: hand written CSS with the shared @adsum/tokens design
tokens. Deployed to Cloudflare Pages. Exact versions in the table at the bottom.
...
| @xyflow/react (React Flow) | Organigramme publié lecture seule | 12.11.2 |
| dagre | Disposition du graphe | 0.8.5 |
| qrcode | Génération de QR | 1.5.4 |
| @adsum/tokens | Design tokens partagés | 0.1.0 |
| Cloudflare Pages | Hébergement (via `wrangler`) | build statique `dist/` |
